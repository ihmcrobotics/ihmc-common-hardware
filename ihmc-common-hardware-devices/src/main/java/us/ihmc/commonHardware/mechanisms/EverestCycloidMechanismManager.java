...
import us.ihmc.commonHardware.devices.etherCATDevices.novanta.YoEverestMotorController;
 ...
 */
public class EverestCycloidMechanismManager implements MechanismManagerInterface
{
    ...
    private final YoEverestMotorController everestMotorController;
    ...
    public EverestCycloidMechanismManager(double jointOffset,
                                   double jointLimitLower,
                                   double jointLimitUpper,
                                   String jointName,
                                   YoEverestMotorController motorController,
                                   YoDouble yoTime,
                                   double estimatorDT,
    ...
    {
        this(jointOffset, jointLimitLower, jointLimitUpper, jointName, motorController, yoTime, estimatorDT, masterDoPDControlOnTwitter, torqueBreakFrequency,
     ...
     * @param motorController      The {@code YoEverestMotorController} for the cycloid
                                   ...
                                   YoRegistry parentRegistry)
    {
        ...
        this.everestMotorController = motorController;
    ...
    {
        everestMotorController.setEnableDrive(false);
        everestMotorController.setDesiredMotorTorque(0.0);
        everestMotorController.setDesiredMotorDamping(0.0);
        everestMotorController.setDesiredMotorStiffness(0.0);
        everestMotorController.write();
    }
    ...
    public void read(LowLevelState measuredJointDataToPack)
    {
        ...
        if (statorTemperature.getValue() > everestMotorController.getMaxMotorTemperature())
        {
            if (!isStatorAboveShutDownTemperature.getValue())
            {
                LogTools.info("OVERHEAT ALERT: " + this.jointName + " stator temperature is " + statorTemperature.getValue() + " deg C. "
                        + "Max allowed stator temperature: " + everestMotorController.getMaxMotorTemperature() + " deg C.");
            }
            isStatorAboveShutDownTemperature.set(true);
        }
        if (statorTemperature.getValue() > everestMotorController.getRecommendedMotorTemperature())
        {
            if (!isStatorAboveRecommendedTemperature.getValue())
            {
                LogTools.info("OVERHEAT WARNING: " + this.jointName + " stator temperature is " + statorTemperature.getValue() + " deg C. "
                        + "Max recommended stator temperature: " + everestMotorController.getRecommendedMotorTemperature() + " deg C.");
            }
        ...
        double jointPosition = computeJointPosition(everestMotorController.getMeasuredOutputPosition(), yoJointOffset.getValue());
        this.measuredActuatorData.setPosition(jointPosition);
        this.measuredActuatorData.setVelocity(everestMotorController.getMeasuredOutputVelocity());
        this.measuredActuatorData.setTorque(everestMotorController.getMeasuredTorque());
    ...
    public void write(JointDesiredOutputReadOnly desiredJointData)
    {
        ...
        {
            double jointPosition = computeJointPosition(everestMotorController.getMeasuredOutputPosition(), yoJointOffset.getValue());
            double jointVelocity = everestMotorController.getMeasuredOutputVelocity();
        ...
        double gearRatio = everestMotorController.getGearRatio();
        double desiredOutputPosition = computeOutputPosition(this.desiredActuatorData.getPosition(), yoJointOffset.getValue());
        double desiredMotorPosition = everestMotorController.getDesiredMotorPosition();
        double kt = everestMotorController.getTorqueConstant();

        // We want to do this because it's way computationally cheaper
        double reflectedMultiplier = 1.0 / (gearRatio * gearRatio * kt);

        everestMotorController.setDesiredMotorPosition(desiredMotorPosition);
        everestMotorController.setDesiredMotorVelocity(this.desiredActuatorData.getVelocity() * gearRatio);
        everestMotorController.setDesiredMotorTorque(filteredDesiredTau.getDoubleValue() / gearRatio);
        everestMotorController.setDesiredMotorStiffness(this.desiredActuatorData.getStiffness() * reflectedMultiplier);
        everestMotorController.setDesiredMotorDamping(this.desiredActuatorData.getDamping() * reflectedMultiplier);

        everestMotorController.setMaxPositionFeedbackError(maxPositionFeedbackError * gearRatio);
        everestMotorController.setMaxVelocityFeedbackError(maxVelocityFeedbackError * gearRatio);
    ...
    {
        everestMotorController.clearFaults();
    }

    /**
     * This class is only used as an alternative to the low-level raw encoder zeroing used in {@code YoEverestMotorController}
     */
    @Override
    public void updateJointOffset()
    {
        yoJointOffset.set(-1.0 * everestMotorController.getMeasuredOutputPosition());
    }

    public void zeroAgainstLimit(double limit)
    {
        everestMotorController.zeroEncodersWithOffset(limit);
    }
    ...
    {
        return everestMotorController.isMotorFaulted();
    }
    ...
    public void setEnableMotors(boolean enableMotors)
    {
        ...
        everestMotorController.setEnableDrive(enableMotors);
    }

    @Override
    public void setEnableCompensationEfforts(boolean enable)
    {
        everestMotorController.setEnableCompensationCurrents(enable);
    }
    ...
    {
        return Math.abs(everestMotorController.getMeasuredMotorCurrent());
    }
    ...
    {
        return everestMotorController.getMotorTemperature();
    }
    ...
    {
        return everestMotorController.getMaxActuatorTorque();
    }
    ...
    {
        everestMotorController.setOutputPositionBreakFrequency(breakFrequency);
    }

    @Override
    public void setVelocityBreakFrequency(double breakFrequency)
    {
        everestMotorController.setOutputVelocityBreakFrequency(breakFrequency);
    }

    public void setMotorPositionBreakFrequency(double breakFrequency)
    {
        everestMotorController.setMotorPositionBreakFrequency(breakFrequency);
    }

    public void setMotorVelocityBreakFrequency(double breakFrequency)
    {
        everestMotorController.setMotorVelocityBreakFrequency(breakFrequency);
    }

    @Override
    public boolean isDynamicBrakingEnabled()
    {
        return everestMotorController.isDynamicBrakingEnabled();
    }
}
